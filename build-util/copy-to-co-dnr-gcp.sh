...
syncFiles() {
  ...
  if [ -z "${answer}" -o "${answer}" = "Y" -o "${answer}" = "y" ]; then
    ...
    if [ "${copyToLatest}" = "yes" ]; then
      ...
      echo ""
      echo 'Remember to run with -l option to upload to the "latest" folder.'
      echo ""
    fi
  fi
  return ${exitStat}
...
dryrun=""
gsFolderLatest="gs://opencdss.state.co.us/tstool/latest/doc-dev"
gsFolderVersion="gs://opencdss.state.co.us/tstool/${tstoolVersion}/doc-dev"
